...
| **Chlorophyll and dissolved oxygen as gridded Fields** | Still right, and chlorophyll arrived by another route: not as a Field but as an *observation* from 49 BGC-Argo floats, drawn on its own because no gridded chlorophyll shares this timeline. INCOIS's own ocean-colour products end 2006-03-21 and 2020-05-01. Oxygen remains out - measured, **zero** BGC casts in this window carry usable oxygen under the project's own QC rules. *Amended 2026-09-15:* both halves are now wrong. Copernicus `GLOBAL_ANALYSISFORECAST_BGC_001_028` publishes daily chlorophyll and oxygen at 0.25 degree covering the whole window, behind the same free account the currents use, and 45 BGC-Argo floats in the box carry oxygen flagged good over the 36-step window (the zero was for the twelve-step window). See `docs/plan/06` |
...
| **HF-radar and ADCP** | India runs five HF-radar pairs and its OMNI buoys measure currents. `services.incois.gov.in` has no route from here at all, and **zero rows** in the public GTS feed carry current components for this region. A data-policy fact, not an architecture gap. *Note, 2026-09-15:* INCOIS Data Holdings lists HF-radar data as "Registered access through Website" and moored-buoy data as "Public Access with only visualisation option", so using either needs a data request to INCOIS. The owner decided not to make one. See `docs/plan/06`. |
